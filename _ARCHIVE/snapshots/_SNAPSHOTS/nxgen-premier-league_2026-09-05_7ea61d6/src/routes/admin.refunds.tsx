        ...
        {row.status === "pending" && (
          <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 12, borderTop: "1px solid var(--line)", paddingTop: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button type="button" className="btn btn-gold btn-sm" onClick={() => review(row, "approved")}>Approve</button>
              <button type="button" className="btn btn-ghost btn-sm" onClick={() => review(row, "denied")}>Deny</button>
            </div>
          </div>
        )}
      </div>
