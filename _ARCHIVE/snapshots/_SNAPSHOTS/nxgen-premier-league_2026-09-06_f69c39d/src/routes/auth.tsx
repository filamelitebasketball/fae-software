...
function AuthPage() {
  ...
  const handleOAuth = async (provider: "google" | "apple") => {
    ...
    const authReturn = `${window.location.origin}/auth?redirect=${encodeURIComponent(returnTo)}`;
    const result = await lovable.auth.signInWithOAuth(provider, { redirect_uri: authReturn });
    if (result.error) {
      setLoading(false);
