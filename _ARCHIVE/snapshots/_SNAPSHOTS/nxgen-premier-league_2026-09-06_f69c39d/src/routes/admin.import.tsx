...
function ImportPage() {
  ...
  const runImport = async () => {
    ...
    setBusy(true);
    try {
      const { data: existingTeams } = await supabase.from("teams").select("id,name,division");
      ...
      const { data: existingPlayers } = await supabase.from("players").select("id,name,division");
