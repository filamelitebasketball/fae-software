...
type Sponsor = { name: string; logo_url: string; website_url: string; active: boolean };
...
const DEFAULTS: Settings = {
  ...
  division_open: Object.fromEntries(DIVISIONS.map((d) => [d.key, true])),
  division_fees: { "rising-stars": 0, legacy: 0, "3x3": 0, kotc: 0 },
  hero_badge_text: "Season 2026 · Now Open",
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data) {
        ...
        (
          ["livestream_url", "site_announcement", "season_label", "hero_badge_text", "payment_note", "court_data_packages", "hero_headline", "hero_subheadline",
            "hero_description", "about_text", "social_instagram", "social_facebook", "social_youtube",
          ...
          <div style={panel}>
            {tab === "General" && (
              <>
                ...
                <Field label="Season label" value={values.season_label} onChange={(v) => setField("season_label", v)} />
              </>
            )}
            ...
            {tab === "Content" && (
              <>
                ...
                <Field label="Hero description" value={values.hero_description} onChange={(v) => setField("hero_description", v)} textarea />
