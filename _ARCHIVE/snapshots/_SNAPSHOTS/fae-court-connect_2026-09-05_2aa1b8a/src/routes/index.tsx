...
import { CountUp } from "@/components/fae/CountUp";
...
function Hero() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setPastCue(window.scrollY > 120);
      if (reduceMotion) return;
      // Parallax: heading and stats drift up slightly slower than the scroll
      if (titleRef.current) titleRef.current.style.transform = `translateY(${(y * 0.12).toFixed(1)}px)`;
      if (statsRef.current) statsRef.current.style.transform = `translateY(${(y * 0.05).toFixed(1)}px)`;
    };
    const onScroll = () => {
      if (!ticking) {
        ticking = true;
  ...
  return (
    <section className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-4 pt-16">
      {/* Court footage behind the hero. Muted + playsInline so mobile browsers
          will autoplay it; poster-less because the first frame is near-black
          and the veil covers it either way. Held at 35% under a dark veil so
          the headline keeps its contrast. */}
      <video
        className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-35 motion-reduce:hidden"
        src="/hero-bg.mp4"
        autoPlay
        muted
        loop
      ...
      <div className="relative z-10 mx-auto flex max-w-5xl flex-col items-center text-center">
        ...
        <Reveal delay={80}>
          <h1
            ref={titleRef}
            className="mt-8 font-display font-black uppercase leading-[0.95] tracking-tight text-foreground will-change-transform [font-size:clamp(3rem,11vw,9rem)]"
          >
            F.A.E. Court
          </h1>
        </Reveal>
        ...
        <Reveal delay={400} className="mt-14 w-full max-w-2xl">
          <div ref={statsRef} className="grid grid-cols-2 gap-y-6 border-t border-border pt-7 will-change-transform sm:grid-cols-4">
            {[
              { value: 3, suffix: "", label: "Sports" },
              { value: 6, suffix: "", label: "Playing surfaces" },
              { value: 20, suffix: " hrs", label: "Open daily" },
              { value: 5, suffix: ".0★", label: "On Google" },
            ].map((stat) => (
              <div key={stat.label}>
    ...
    <section className="mx-auto max-w-7xl px-4 py-24 sm:px-6">
      ...
      <div className="mt-12 grid gap-5 md:grid-cols-3">
        {sports.map((sport, i) => {
          ...
          return (
            <Reveal key={sport.key} delay={i * 80} className="h-full">
  ...
  return (
    <section className="border-y border-border bg-surface-1 py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="md:grid md:grid-cols-[minmax(0,2fr)_minmax(0,3fr)] md:gap-12">
          <Reveal className="md:self-start">
            <div className="md:sticky md:top-24">
              ...
              <p className="mt-5 max-w-sm text-sm leading-relaxed text-muted-foreground">
                Two squads, one home floor. Scroll through the programs, then book the same surface they train on.
              </p>
            </div>
          </Reveal>

          <div className="mt-12 grid gap-6 md:mt-0">
