...
function LedgerConsoleInner() {
  ...
  async function topUp(amount: number, method: string) {
    ...
    const { error: wErr } = await supabase.rpc("apply_wallet_delta", {
      _user_id: selected.id, _balance_delta: amount, _tab_delta: 0,
    });
    if (wErr) {
  ...
  async function charge(item: InventoryItem, method: "wallet" | "cash" | "tab") {
    ...
    if (method === "tab" && !wallet?.loyal_tab_enabled) {
      return toast.error("Player is not enrolled in loyal tab");
    }
    const { data: entry, error } = await supabase.from("player_ledger_entries").insert({
    ...
    if (method === "wallet" || method === "tab") {
      const { error: wErr } = await supabase.rpc("apply_wallet_delta", {
        ...
        _balance_delta: method === "wallet" ? amt : 0,
        _tab_delta: method === "tab" ? item.price_cents : 0,
      });
      if (wErr) {
        await supabase.from("player_ledger_entries").delete().eq("id", entry.id);
        selectPlayer(selected);
  ...
  async function endCafe(method: "wallet" | "cash" | "tab") {
    ...
    if (method === "wallet" || method === "tab") {
      const { error: wErr } = await supabase.rpc("apply_wallet_delta", {
        _user_id: selected.id,
        _balance_delta: method === "wallet" ? -charge : 0,
        _tab_delta: method === "tab" ? charge : 0,
      });
      if (wErr) toast.error(`Session closed but the wallet did not move: ${wErr.message}`);
    }
