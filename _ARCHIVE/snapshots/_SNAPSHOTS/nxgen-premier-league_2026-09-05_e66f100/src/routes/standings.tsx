...
export const Route = createFileRoute("/standings")({
  head: () => ({
    ...
    links: [{ rel: "canonical", href: "https://nxgenpremierleague.lovable.app/standings" }],
  }),
...
function StandingsPage() {
  ...
  useEffect(() => {
    (async () => {
      const [t, g] = await Promise.all([
        ...
        supabase
          .from("games")
          .select("division, status, home_team_id, away_team_id, home_team_name, away_team_name, home_score, away_score"),
      ]);
  ...
  // A division's regular season is complete when it has games and all are final.
  const seasonComplete = useMemo(
    () => (div: string) => {
      const divGames = games.filter((g) => g.division === div);
      return divGames.length > 0 && divGames.every((g) => g.status === "final");
    },
    [games],
  );
  ...
  function StandingsTable({ rows, complete }: { rows: Standing[]; complete: boolean }) {
    // Playoff cutoff: teams entered minus 4 advance.
    const cutoff = Math.max(rows.length - 4, 0);
    return (
      <div className="card" style={{ padding: "8px 20px", marginBottom: 12 }}>
        ...
        <table className="w-full text-sm" style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {rows.map((t, i) => {
              ...
              const advanced = complete && i < cutoff;
              const eliminated = complete && i >= cutoff;
              return (
        ...
        <p style={{ marginTop: 10, marginBottom: 10, fontFamily: "var(--fm)", fontSize: 9.5, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--silver-d)" }}>
          Ties broken by head-to-head record, then point differential. Playoffs: teams entered minus 4 advance
          {rows.length ? ` (${rows.length}→${cutoff})` : ""}.
        </p>
      </div>
    );
  }
  ...
  return (
    <NxPage eyebrow="Rankings" title="Standings" intro="Wins, losses, win percentage and point differential across every NXGEN division.">
      <div className="tab-row" role="tablist" style={{ marginBottom: 32 }}>
        {["All", ...DIVISIONS].map((d) => (
