"""DinkCut IO v3.5 · © 2026 LINKMEIO. All rights reserved.
...
longest rally, the full game and/or one clip per rally, and shares them by QR code on the venue WiFi.
    ...
    python dinkcut_io.py                      desktop app
    python dinkcut_io.py --cli match.mp4      command line (see --help)
    python dinkcut_io.py --selftest           quick logic check
"""
...
APP, VERSION, OWNER = "DinkCut IO", "3.5", "LINKMEIO"
...
# storage keeps the pre-rename name, so settings and trial clocks carry over from Highlight Studio IO
...
def audio_hits(path, sensitivity=6, sr=16000):
    """Sharp 1-4 kHz onsets, the paddle 'pop', as (hits, soft) times in s: hits at `sensitivity` (1 strict .. 10 loose),
    soft = the quieter pops two steps looser, which catch the soft serve and dinks."""
    ...
    med = np.median(rise)
    mad, loud, spacing = np.median(np.abs(rise - med)) + 1e-9, loge > np.percentile(loge, 50), int(0.15 * sr / hop)
    def pick(s):
        out, last = [], -10**9
        for i in np.flatnonzero((rise > med + (13 - s) * 1.4826 * mad) & loud):
            if i - last >= spacing:
                out.append(i * hop / sr)
                last = i
        return out
    # ponytail: soft level capped at 8, the loosest checked on real footage without joining two points; retune in the pilot
    return pick(sensitivity), pick(max(sensitivity, min(sensitivity + 2, 8)))
...
def find_rallies(hits, duration, gap=2.5, min_hits=3, pre=1.8, post=1.2, active=None, soft=(), lead=4.0):
    """Rallies as [(start, end, pops)]: runs of pops under `gap` s apart, >= min_hits pops with 2+ of them loud `hits`.
    Soft pops (the soft serve, dinks) join a run and pull its start back, at most `lead` s before its first loud hit;
    it ends at its last loud hit. Padded by pre/post, overlaps merged."""
    h = np.sort(np.asarray(hits, float))
    pops = sorted([(t, True) for t in hits] + [(t, False) for t in soft if not (len(h) and np.abs(h - t).min() <= 0.15)])
    runs, segs = [], []                          # (a soft pop within 0.15 s of a hit is the same pop)
    for t, big in pops:
        if active is None or active(t):
            if runs and t - runs[-1][-1][0] <= gap:
                ...
                runs.append([(t, big)])
    for run in runs:
        loud = [t for t, b in run if b]
        if len(loud) < 2 or len(run) < min_hits:
            continue
        a, n = max(0.0, max(run[0][0], loud[0] - lead) - pre), len(run)
        b = min(duration, loud[-1] + post) if duration else loud[-1] + post
...
# ---------------------------------------------------------------- watermark
def overlays(vf, W, H, vertical, logo=None):
    """ffmpeg args for the video filter, plus the logo small at the bottom right when there is one."""
    if not logo:
        return ["-vf", vf, "-map", "0:v:0", "-map", "0:a?"]
    m = round(H * 0.03)
    return ["-i", str(logo), "-filter_complex",
            f"[0:v]{vf}[v0];[1:v]scale={round(W * (0.2 if vertical else 0.1))}:-1[lg];[v0][lg]overlay=W-w-{m}:H-h-{m}[v1]",
            "-map", "[v1]", "-map", "0:a?"]
...
def build_reel(src, segs, dst, vs, vertical, logo, prog=None):
    """Join rallies into one video (crop to 9:16 when vertical), logo small at the bottom right."""
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for k, (a, b, _) in enumerate(segs):
            ...
            args += overlays(vf, W, H, vertical, logo)
...
def full_game(src, dst, vs, dur, prog=None):
    if vs["fmt"] == "copy":                         # stream copy: seconds, not minutes
        ff(["-i", str(src), "-c", "copy", "-movflags", "+faststart", str(dst)])
        return
    ff(["-i", str(src), "-vf", vfilter(vs), "-map", "0:v:0", "-map", "0:a?", *encoder(vs), str(dst)], dur, prog)


# ---------------------------------------------------------------- the whole job
def process(src, out_dir, opt, log=print, step=lambda frac, text: None):
    """Find the rallies in src, write the chosen exports to <out_dir>/<name>-highlights. Returns (folder, files, rallies)."""
    if trial_left() == 0:
    ...
    dur = duration_of(src)
    hits, soft = audio_hits(src, opt["sensitivity"])
...
def run_app():
    ...
    ctk.CTkLabel(side, text=APP, font=F(17, True, "Bahnschrift"), text_color=INK, anchor="w").pack(fill="x", padx=18, pady=(20, 0))
    ...
    def draw_hero(e=None):
        ...
        hero.create_text(24, 72, text="Pick a video, press Make highlights. QR download included.",
    ...
    PAGES = (("Home", "\u2302", "Preset and the match video"), ("Exports", "\u25a6", "What the player takes home"),
             ("Video", "\u25b6", "Format, size and smoothness"),
    ...
    try:                                        # the packaged exe shows an "Opening..." splash while it unpacks
        import pyi_splash
        pyi_splash.close()
    except ImportError:
        pass
...
def selftest():
    ...
    # soft pops: the serve and dinks join two loud hits 2.8 s apart into one rally that starts at the serve
    assert find_rallies([20, 22.8], 60) == [] and find_rallies([5], 60, soft=[4, 6, 7]) == []   # 2+ loud hits needed
    assert find_rallies([20, 22.8], 60, soft=[19.0, 20.05, 21.4, 23.9]) == [(17.2, 24.0, 5)]    # 20.05 = the same pop
    assert find_rallies([17, 18, 19], 60, soft=[10, 12, 14, 16]) == [(11.2, 20.2, 7)]          # start pulled back 4 s max
