...
const DB = {
  ...
  staff: [{ n: 'filamelitebasketball', role: 'Owner (admin)' }, { n: 'Front desk 1', role: 'Staff' }, { n: 'Court marshal', role: 'Staff' }]
};
const TASKS = [
...
function ownerLogin() {
  const u = ($('#oUser').value || '').trim().toLowerCase().replace('@gmail.com', '');
  if (u !== 'filamelitebasketball') return $('#oErr').classList.remove('hide');
  S.admin = true; closeDlgs(); toast('Owner console unlocked'); go('admin');
}
...
function renderAdmin() {
  ...
  <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
    <div class="flex items-center gap-3"><img src="img/p-mark.svg" alt="" class="h-11"><div><h1 class="font-bold text-2xl leading-none">Owner console</h1><p class="text-xs text-muted mt-1">Playhouse Pickle · signed in as <span class="text-lime font-semibold">filamelitebasketball</span></p></div></div>
